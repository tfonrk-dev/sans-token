...
export default function Presale() {
  const t = useT();
  const perks = [
    `${t.presale.perks[0]}: ${SITE.presale.price} → ${SITE.presale.listing}`,
    `${SITE.presale.bonus} ${t.presale.perks[1]}`,
    t.presale.perks[2],
    t.presale.perks[3],
  ];
      ...
      <div className="mx-auto max-w-7xl px-5 lg:px-8">
        <div className="grid items-center gap-8 rounded-chunk bg-gradient-to-br from-sea to-sea-dark p-8 shadow-pop sm:p-10 lg:grid-cols-2">
          ...
          <div className="text-white">
            ...
            <h2 className="mt-4 font-display text-4xl font-bold">{t.presale.title}</h2>
            <p className="mt-3 max-w-xl font-semibold text-white/90">{t.presale.desc}</p>
          ...
          <div className="rounded-chunk bg-cream p-6 shadow-pop sm:p-7">
            <div className="flex items-baseline justify-between border-b border-sky-100 pb-3">
              <span className="text-[11px] font-extrabold uppercase tracking-wide text-mute">{t.presale.token}</span>
              <span className="font-display text-xl font-bold text-navy">$SANS · TON</span>
            </div>
            <dl className="mt-4 space-y-3 text-sm font-bold">
              <Row k={t.presale.priceLabel} v={`${SITE.presale.price} / SANS`} />
              <Row k={t.presale.listLabel} v={SITE.presale.listing} highlight />
              <Row k={t.presale.minLabel} v={SITE.presale.minBuy} />
              <Row k={t.presale.supplyLabel} v={`${SITE.totalSupply} SANS`} />
            ...
            >
              {t.presale.cta} <ArrowRight size={18} />
            </a>
            <p className="mt-3 text-center text-[11px] font-bold text-mute">{t.presale.note}</p>
          </div>
        </div>
      </div>
    </section>
